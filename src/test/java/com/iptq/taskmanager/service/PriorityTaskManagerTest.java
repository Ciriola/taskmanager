...
class PriorityTaskManagerTest {
    ...
    @Test
    public void priorityPolicyEviction_whenCapacityIsExceeded() {
        TaskManager taskManager = new PriorityTaskManager(3);

        Process first = new Process(1, Priority.MEDIUM);
        Process second = new Process(2, Priority.LOW);
        ...
        assertThat(taskManager.list().size()).isEqualTo(3);
        assertThat(taskManager.list().containsAll(Arrays.asList(first, third, fourth))).isTrue();
    }

    @Test
    public void processRemovedFromTM_whenKilled() {
        TaskManager taskManager = new PriorityTaskManager(2);
    ...
    public void allProcessesGroupRemovedFromTM_whenKillGroup() {
        TaskManager taskManager = new PriorityTaskManager(4);
